...
import { Anexo, Convenio } from '../domain';
import { ConvenioState } from './convenio-state';

@Component({
  selector: 'anexo-convenio',
  template: `
      ...
      <div class="col-sm-12 col-lg-9">
          ...
          <tbody>
            <tr *ngFor="let anexo of anexos | async; let i = index;">
              ...
              <td style="text-align: right;">
                <a href="api\/anexo\/convenio\/{{anexo.id}}" target="_blank"><span class="glyphicon glyphicon-download" aria-hidden="true"></span></a>
        ...
        <ng-template #listaVazia>
          <div style="margin-top: 5px;">
            Esse convênio não possui arquivos. Clique no botão "Enviar arquivo" para adicionar um arquivo.
          </div>
        </ng-template>
      </div>
    </div>
...
export class AnexoConvenioComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.convenioSubscription = this.convenioState.convenio
      .subscribe((convenio) => {
        this.convenio = convenio;
        if (convenio) {
          this.getAnexosConvenio();
        }
      });
    this.anexos = this.convenioState.anexosBehavior;
  }
  ...
  getAnexosConvenio() {
    this.convenioState.getAnexosConvenio();
  }
  ...
  salvarAnexo(evento) {
    ...
    if (fileList.length > 0) {
      ...
      const req = new HttpRequest('POST', 'api/anexo/convenio?convenioId=' + this.convenio.id, formData, {
        reportProgress: true,
      });
