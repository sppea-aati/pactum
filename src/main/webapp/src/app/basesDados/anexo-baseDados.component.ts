...
import { Anexo, BaseDados } from '../domain';
import { BaseDadosState } from './baseDados-state';

@Component({
  selector: 'anexo-baseDados',
  template: `
      ...
      <div class="col-sm-12 col-lg-9">
          ...
          <tbody>
            <tr *ngFor="let anexo of anexos | async; let i = index;">
              ...
              <td style="text-align: right;">
                <a href="api\/anexo\/baseDados\/{{anexo.id}}" target="_blank"><span class="glyphicon glyphicon-download" aria-hidden="true"></span></a>
        ...
        <ng-template #listaVazia>
          <div style="margin-top: 5px;">
            Essa base de dados não possui arquivos. Clique no botão "Enviar arquivo" para adicionar um arquivo.
          </div>
        </ng-template>
      </div>
    </div>
...
export class AnexoBaseDadosComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.baseDadosSubscription = this.baseDadosState.baseDados
      .subscribe((baseDados) => {
        this.baseDados = baseDados;
        if (baseDados) {
          this.getAnexosBaseDados();
        }
      });
    this.anexos = this.baseDadosState.anexosBehavior;
  }
  ...
  getAnexosBaseDados() {
    this.baseDadosState.getAnexosBaseDados();
  }
  ...
  salvarAnexo(evento) {
    ...
    if (fileList.length > 0) {
      ...
      const req = new HttpRequest('POST', 'api/anexo/baseDados?baseDadosId=' + this.baseDados.id, formData, {
        reportProgress: true,
      });
